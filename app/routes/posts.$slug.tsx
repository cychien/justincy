...
export function meta({ data }: Route.MetaArgs) {
  if (!data?.post) {
    return [{ title: '文章未找到 - 我的部落格' }];
  }
  return [
    { title: `${data.post.meta.title} - 我的部落格` },
...
export async function loader({ params }: Route.LoaderArgs) {
  ...
  const post = getPostBySlug(params.slug!, 'post');
  if (!post) {
    ...
    <article className="animate-fade-in mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <header className="mb-12">
        <div className="mb-4 flex flex-wrap items-center gap-3">
          <span className="inline-flex items-center rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700 dark:bg-indigo-950 dark:text-indigo-400">
            文章
          </span>
      ...
      {/* Content */}
      <div
        className="prose prose-lg dark:prose-invert prose-headings:font-bold prose-headings:tracking-tight prose-h1:text-3xl prose-h2:text-2xl prose-h3:text-xl prose-p:leading-7 prose-p:text-gray-700 dark:prose-p:text-gray-300 prose-a:text-indigo-600 dark:prose-a:text-indigo-400 prose-blockquote:border-l-4 prose-blockquote:border-indigo-300 prose-blockquote:bg-indigo-50/50 prose-blockquote:py-1 prose-blockquote:pl-6 prose-blockquote:italic dark:prose-blockquote:border-indigo-700 dark:prose-blockquote:bg-indigo-950/30 prose-code:rounded prose-code:bg-gray-100 prose-code:px-1.5 prose-code:py-0.5 prose-code:text-sm dark:prose-code:bg-gray-800 prose-pre:rounded-xl prose-pre:bg-gray-900 prose-pre:p-4 max-w-none"
